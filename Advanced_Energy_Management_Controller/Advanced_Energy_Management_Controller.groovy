...
def mainPage() {
    dynamicPage(name: "mainPage", title: "Energy Management Core", install: true, uninstall: true) {
        ...
        section("Live Financial & Health Dashboard") {
            ...
            def appliances = ["refrigerator": "Refrigerator", "chestFreezer": "Chest Freezer", "washerDryer": "Washer/Dryer", "dishwasher": "Dishwasher", "microwave": "Microwave"]
            ...
            def cycleAppliances = ["washerDryer": "Washer/Dryer", "dishwasher": "Dishwasher", "microwave": "Microwave"]
            ...
            // Compressor Cycle Stats (7-Day)
            statusText += "<b>Compressor Cycle Stats (7-Day)</b><br><table style='width:100%; border-collapse: collapse; font-size: 13px; font-family: sans-serif; background-color: #fcf8e3; border: 1px solid #ccc; margin-bottom: 15px;'>"
            statusText += "<tr style='background-color: #faebcc; border-bottom: 2px solid #ccc; text-align: left;'><th style='padding: 8px;'>Appliance</th><th style='padding: 8px;'>State</th><th style='padding: 8px;'>Avg Cycle</th><th style='padding: 8px;'>7-Day Runs</th></tr>"
            ...
            compressorAppliances.each { key, name ->
                ...
                def avgCycleStr = runCount > 0 ? "${Math.round(totalMins / runCount)} min" : "N/A"
                 
                statusText += "<tr style='border-bottom: 1px solid #ddd;'><td style='padding: 8px;'><b>${name}</b></td><td style='padding: 8px;'>${isRunning}</td><td style='padding: 8px;'>${avgCycleStr}</td><td style='padding: 8px;'>${runCount}</td></tr>"
            }
            statusText += "</table>"
        ...
        section("Mode-Based Vampire Power Shutdown") {
            ...
            paragraph "Note: The Refrigerator and Chest Freezer are permanently excluded from mode-based shutdowns to protect perishables."
        }
        ...
        section("Appliance Scheduling & Shutdown") {
            ...
            paragraph "Note: Refrigerator and Chest Freezer are permanently excluded from schedule shutdowns."
        }
        ...
        section("Refrigerator", hideable: true, hidden: true) {
            ...
            input "refrigeratorTemp", "capability.temperatureMeasurement", title: "Temperature Sensor", required: false
            input "refrigeratorTempThreshold", "number", title: "Max Temperature Alert Threshold (°F/°C)", defaultValue: 42, required: false
        ...
        section("Chest Freezer", hideable: true, hidden: true) {
            ...
            input "chestFreezerTemp", "capability.temperatureMeasurement", title: "Temperature Sensor", required: false
            input "chestFreezerTempThreshold", "number", title: "Max Temperature Alert Threshold (°F/°C)", defaultValue: 15, required: false
...
def clearAllData() {
    def appliances = ["refrigerator", "chestFreezer", "washerDryer", "dishwasher", "microwave"]
...
def executeVampireShutdown() {
            ...
            if (sw) {
                ...
                if (currentPower > safeThreshold) {
                    ...
                    if (sw.currentValue("switch") != "off") {
                        ...
                        state["${key}_context"] = "Vampire Shutdown"
                        sw.off()
    ...
    // Retry in 15 minutes if someone is doing laundry when the house shifts to Away/Night
    if (needsRetry) {
        runIn(900, executeVampireShutdown)
    }
}
...
def executeVampireRestore() {
    ...
    targets.each { target ->
        if (target.enabled) {
            ...
            if (sw) {
                state["${key}_systemActionPending"] = true
                state["${key}_context"] = "Vampire Restore"
                sw.on()
...
def triggerTurnOff() {
    // Refrigerator & Chest Freezer explicitly removed from schedule shutdown targets
    def appliances = ["washerDryer", "dishwasher", "microwave"]
    def safeThreshold = settings["safeShutdownThreshold"]?.toString()?.toDouble() ?: 15.0
    def needsRetry = false
    ...
    // Retry in 15 minutes if someone is mid-cycle
    if (needsRetry) {
...
def triggerTurnOn() {
    // Refrigerator & Chest Freezer explicitly removed from schedule restore targets
    def appliances = ["washerDryer", "dishwasher", "microwave"]
    def kwhRate = settings["costPerKwh"]?.toString()?.toDouble() ?: 0.15
    ...
    appliances.each { key ->
        ...
        if (sw) {
            state["${key}_systemActionPending"] = true
            // Only overwrite context if we aren't coming from an override
...
def powerHandler(evt) {
    ...
    appliances.each { key, name ->
        if (settings["${key}Power"]?.id == meterId) {
            ...
            // 2. State & Debounced Cycle Tracking 
            ...
            if (isCurrentlyRunning) {
                ...
                // If the power spiked back up during our pause window, cancel the completion schedule
                if (state["${key}_stopPending"]) {
                    state["${key}_stopPending"] = false
                    unschedule("checkCycleComplete_${key}")
                }
            } else {
                // If it dropped below the threshold, start the countdown timer
            ...
            if (settings["${key}AutoOff"]) {
                ...
                if (!allowedModes || allowedModes.contains(location.mode)) {
                    ...
                    if (sw && sw.currentValue("switch") != "off") {
                        state["${key}_systemActionPending"] = true
                        state["${key}_context"] = "Cycle Complete (Auto-Off)"
                        sw.off()
                        log.info "${name} has been automatically turned OFF after completing its cycle."
                    }
                } else {
